...
const ModeSwitcher = () => {
    ...
    const { accounts, shared_accounts, account_id, switchAccountMode, selected_account } =
        useAppModeContext()
    ...
    const picked_shared_account = shared_accounts?.filter(
        (shared: any) => shared.account_id === selected_account?.id,
    )
                        ...
                        {accounts?.map((account: Account) => {
                            return (
                                <StyledDropDownMenuItem
                                    key={account.id}
                                    onClick={() => switchAccountMode(account)}
                                >
                                    <StyledImg src={account_mode_icon[account.account_type]} />

                                    <StyledDropdownValue>
                                        <TypographyPrimary
                                            value={strCutter(account.name, 24, true)}
                                            size={'xs-small'}
                                            semiBold
                                        />
                        ...
                        {shared_accounts?.map((account: any) => {
                            return (
                                <>
                                    ...
                                    >
                                        <StyledImg
                                            src={account_mode_icon[account?.assigned_account_type]}
                                        />

                                        <TypographyPrimary
                                            value={`${strCutter(
                                                account.assigned_account_name,
                                                24,
                                                true,
                                            )}'s Team`}
                                            size={'small'}
...
const StyledDropDownMenuItem = styled(Item)<{ sticky?: boolean; borderBottom?: boolean }>`
    ...
    border-top: ${({ sticky, theme }) =>
        sticky ? `1px solid ${theme.body.secondaryBorderBackground}` : ''};
