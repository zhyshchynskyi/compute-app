                ...
                <StyledTbody {...getTableBodyProps()}>
                    {rows?.map((row: any, index: number) => {
                        ...
                        const hasHiddenContent =
                            hiddenContentAccessor && hiddenContentAccessor(row.original)
                        ...
                        return (
                            <>
                                <StyledTr
                                    ...
                                    bodyRow
                                    isSelected={
                                        row.original.id === selectedRowState ||
                                        (openedRowIndices.includes(index) && hiddenContentAccessor)
                                    }
                                    onClick={() => toggleRow(index)} // Toggle row on click
                                    hasHiddenContent={hasHiddenContent}
                                >
                                    {row.cells.map(
                                        (cell: Cell<Record<string, unknown>>, index: number) => {
                                            return (
                                                <TableCell
                                                    key={index}
                                                    cell={cell}
                                ...
                                {openedRowIndices.includes(index) &&
                                    hiddenContentAccessor && ( // Conditionally render hidden content
                                        <StyledHiddenContent>
                                            {hiddenContentAccessor(row.original)}
                                        </StyledHiddenContent>
                                    )}
                            </>
                        )
                ...
                <PaginationWrapper>
                    ...
                    {page > 2 && (
                        <>
                            ...
                            {page !== 3 && (
                                <PageNumber onClick={() => paginate(page - 2)}>
                                    {page - 2}
                                </PageNumber>
                            )}
                    ...
                    {totalPageState > 1 && page !== totalPageState && (
                        <PageNumber
                            onClick={() => paginate(totalPageState)}
                            active={totalPageState === page}
                        >
                            {totalPageState}
                        </PageNumber>
                    )}
