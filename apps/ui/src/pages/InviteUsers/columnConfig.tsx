...
import { StyledActionWrapper } from 'pages/ApiKeys/columnConfig'
        ...
        Cell: ({ row: { original: data } }: { row: { original: Pick<ColumnRowProps, 'id'> } }) => (
            <StyledActionWrapper>
                <IconButton
                    ...
                    icon={() =>
                        deleting_loading.loading && deleting_loading.id === data.id ? (
                            <Loader size={16} />
                        ) : (
                            <StyledDeleteIcon />
                        )
...
export const renderSharedColumns = ({
    selected_account_id,
    handleSelectAccess,
}: SharedColumnProps) => [
    {
        ...
        Cell: ({
            ...
            row: { original: Pick<ColumnRowProps, 'account_id'> }
        }) => (
            <Checkbox
