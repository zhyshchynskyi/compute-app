...
const AgentRunners = ({ formikField, placeholder }: AgentRunnersProps) => {
  ...
  return (
    <FieldArray name={formikField}>
      {({ insert, remove }) => (
        <StyledFieldsWrapper>
          <TypographyPrimary value={`${placeholder}s`} type={Typography.types.LABEL} size={Typography.sizes.lg} />
          ...
          <StyledButtonWrapper>
            <ButtonSecondary onClick={() => insert(values[formikField].length, '')} size={Button.sizes?.SMALL}>
              + {t('add')}
            </ButtonSecondary>
          </StyledButtonWrapper>
        </StyledFieldsWrapper>
