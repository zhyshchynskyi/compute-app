...
const usePayment = () => {
    ...
    const is_card_added = account?.configs?.payment?.is_card_added
    const sessionId = new URLSearchParams(location.search).get('session_id')
    const cardSessionId = new URLSearchParams(location.search).get('card_session_id')

    const getCustomerId = async () => {
        let customer_id = account?.configs?.payment?.stripe_customer_id
        // if (!customer_id) {
        //     const result = await createCustomer({}, user.email, user.id)
    ...
    const removeCardHandler = async () => {
        ...
        const payment_method_id = account?.configs?.payment?.payment_methods_ids[0]
