...
const Toast: FC<ToastProps> & {
    ...
    useEffect(() => {
        if (open && Number(autoHideDuration) > 0) {
            setAutoHideTimer(Number(autoHideDuration))
        }
    ...
    const iconElement = !hideIcon && icon && getIcon(type, icon, ToastIconSize[iconSize])
              ...
              <div className="l3-style-toast-avatar">
                <Avatar type={Avatar.types.IMG} size={Avatar.sizes.SMALL} src={avatarSrc} rectangle withoutBorder />
                    ...
                    {(toastButtons || deprecatedAction) && (
                        <div className='l3-style-toast-action'>
                            {toastButtons || deprecatedAction}
                        </div>
                    )}
                    {/* {closeable && (
