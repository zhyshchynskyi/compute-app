...
export const datasourceValidationSchema = yup.object().shape({
  datasource_name: yup.string().min(2, 'Too Short!').max(50, 'Too Long!').required('Please enter name'),
})
...
export const fineTuningValidationSchema = yup.object().shape({
  fine_tuning_name: yup.string().min(2, 'Too Short!').max(50, 'Too Long!').required('Please enter name'),
